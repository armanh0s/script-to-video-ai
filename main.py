...
# Naye moviepy (Python 3.14+) ke liye correct import tarika
...
def create_smart_video(script_scenes, total_audio_duration, final_audio_path, video_mode, orientation_tag, output_filename="smart_output.mp4"):
    ...
    try:
        for file_path, duration in downloaded_files:
            # Naye moviepy mein .resize() ki jagah resized() ya resized_to_fit() hota hai, par safely chalaney ke liye normal subclip karenge
            raw_clip = VideoFileClip(file_path)
            clips.append(raw_clip.subclipped(0, min(raw_clip.duration, duration)))
            
        final_video_only = concatenate_videoclips(clips, method="compose")
        final_audio = AudioFileClip(final_audio_path)
        final_video_with_audio = final_video_only.with_audio(final_audio)
